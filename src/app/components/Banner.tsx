...
const BannerOne: React.FC<BannerOneProps> = ({
    ...
    return (
        <section className="relative w-full min-h-[65vh] sm:min-h-screen flex items-center justify-center overflow-hidden font-sans bg-stone-950">
            ...
            <div className="absolute inset-0 z-0">
                <Image 
                    ...
                    fill
                    priority
                    className="object-cover object-center transition-transform duration-10000 hover:scale-105 brightness-110 contrast-105 saturate-[1.05]"
                />
            </div>


            {/* <div className="absolute inset-0 bg-black/30" /> Soft global shade to keep white text readable */}
            <div className="absolute inset-0 z-0 bg-gradient-to-r from-black/80 via-black/40 to-transparent sm:from-black/70 sm:via-black/35 sm:to-transparent" /> {/* Left-aligned text protection */}
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto px-5 py-10 sm:py-20 sm:px-12 flex flex-col justify-between min-h-[65vh] sm:min-h-screen">
                ...
                <div className="max-w-2xl mt-4 sm:mt-0">
                    ...
                    {/* Rating Badge */}
                    <div className="inline-flex items-center gap-1.5 sm:gap-2 bg-black/60 backdrop-blur-md border border-stone-750 rounded-full px-3 py-1 sm:px-3.5 sm:py-1.5 w-fit mb-4 sm:mb-6 shadow-lg">
                        <div className="flex items-center text-amber-400">
                            {[...Array(5)].map((_, i) => (
                                <Star key={i} className="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-current" />
                            ))}
                        </div>
                        <span className="text-[10px] sm:text-xs font-semibold text-stone-100">
                            {ratingText}
                        </span>
                        <span className="h-3 w-px bg-stone-700" />
                        <span className="text-[10px] sm:text-xs text-stone-400 font-light">
                            {reviewsText}
                        </span>
                    </div>

                    <h1 className="text-3xl sm:text-6xl font-bold tracking-tight text-white leading-[1.15] mb-3 sm:mb-6 drop-shadow-[0_2px_15px_rgba(0,0,0,0.5)] font-serif">
                    ...
                    <p className="text-sm sm:text-lg text-stone-200 font-light leading-relaxed mb-6 sm:mb-8 max-w-xl drop-shadow-[0_1px_8px_rgba(0,0,0,0.5)]">
                        {subheading}
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                        <button
                            ...
                            className="group relative flex items-center justify-center gap-2 bg-[#4A5952] hover:bg-[#5b6e65] text-white px-6 py-3 sm:px-8 sm:py-4 rounded-xl font-medium shadow-[0_4px_25px_rgba(74,89,82,0.4)] transition-all duration-300 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer"
                        >
                            <Coffee className="w-4 h-4 transition-transform group-hover:rotate-12" />
                            <span className="text-sm sm:text-base">{primaryCtaText}</span>
                            <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
                        </button>

                        <button
                            onClick={onSecondaryCtaClick}
                            className="flex items-center justify-center gap-2 border border-stone-500 hover:border-[#8BA196] bg-black/25 hover:bg-white/10 text-stone-100 px-6 py-3 sm:px-8 sm:py-4 rounded-xl font-medium backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer"
                        >
                            <MapPin className="w-4 h-4 text-stone-300" />
                            <span className="text-sm sm:text-base">{secondaryCtaText}</span>
                        </button>
                    </div>
                </div>
